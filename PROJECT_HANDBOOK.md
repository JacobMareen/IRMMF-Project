...
**Branching Logic:** Integrated Corruption module into Governance (`EXCO-D6-Q03`), Legal (`SEC-D9-Q02`), and Culture (`HR-D4-Q02`) flows.
...
```
**Question Bank Source:** `app/core/irmmf_bank.json` (curated production bank).  
**Full Bank Export:** `app/core/full_question_bank.json` (generated by `scripts/migrate_excel_to_json.py`).

### Health & Status
Use the system status endpoint (and Settings → System Status card) to confirm backend/database health:
```
GET /api/v1/status
```
Key indicators:
- `database.ok` should be true.
- `question_bank.questions` and `question_bank.answers` should be > 0.
- `migrations.alembic_version` should match the expected revision.
...
*   **Assessment Hub empty:** Check `/api/v1/status` for question bank counts (should be > 0).
