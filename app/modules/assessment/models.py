...
class Archetype(Base):
    """
    Risk Personas / Archetypes definition storage.
    Allows dynamic updates to descriptions and logic without code changes.
    """
    __tablename__ = "dim_archetypes"

    name: Mapped[str] = mapped_column(String(64), primary_key=True)
    description: Mapped[str] = mapped_column(Text, nullable=True)
    strengths: Mapped[List[str]] = mapped_column(ARRAY(String), nullable=True)
    weaknesses: Mapped[List[str]] = mapped_column(ARRAY(String), nullable=True)
    peer_comparison: Mapped[str] = mapped_column(Text, nullable=True)
    
    # Metadata
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), onupdate=utcnow, default=utcnow)
